...
window.onload = function () {
  ...
  function loadGame() {
    ...
    let player = new __WEBPACK_IMPORTED_MODULE_5__components_Player_js__["a" /* Player */]({
      x: 1.5,
      y: 1.5,
      direction: 1,
      papers: papers,
...
class Map {
  ...
  buildMap(trees, bushes) {
    ...
    for (let i = 0; i < this.size * this.size; i++) {
      ...
      let col = i - this.size * row; // Generate the labirinth
...
class Camera {
  ...
  drawSprites(player, map, columnProps) {
    ...
    let resolution = this.resolution; // calculate each sprite distance to player
